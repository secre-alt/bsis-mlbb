    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-5 py-6 sm:grid-cols-2">
      <div>
        ...
        <div className="space-y-2.5">
          {completed.length ? (
            completed.map((m) => <MatchCard key={m.id} match={m} getTeam={getTeam} isResult />)
          ) : (
      ...
      <div>
        ...
        <div className="space-y-2.5">
          {upcoming.length ? (
            upcoming.map((m) => <MatchCard key={m.id} match={m} getTeam={getTeam} />)
          ) : (
  ...
  return (
    <div className="relative overflow-hidden rounded-md border border-ink-800 bg-ink-900 p-4">
      ...
      <div className="mb-3 flex justify-between font-mono text-[9px] font-bold uppercase tracking-wider text-ink-600">
        <span>Match {m.num} · Round {m.round}</span>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {isResult ? (
            <>
              <span className={`font-display text-2xl font-bold ${aWin ? 'text-ember-500' : 'text-ink-700'}`}>{m.scoreA}</span>
              <span className="text-xs text-ink-700">:</span>
              <span className={`font-display text-2xl font-bold ${!aWin ? 'text-ember-500' : 'text-ink-700'}`}>{m.scoreB}</span>
            </>
          ) : (
            <span className="rounded-sm bg-ink-800 px-3 py-1.5 text-[10px] font-bold text-ink-500">VS</span>
          )}
        </div>
        <TeamBlock team={tb} dim={isResult && aWin} reverse />
      </div>
      <div className={`mt-3 text-center text-[9px] font-bold uppercase tracking-wider ${isResult ? 'text-ink-600' : 'text-ember-500'}`}>
        {isResult ? `Final · ${aWin ? ta.abbr : tb.abbr} wins` : `Upcoming · ${m.time || ''}`}
      </div>
    </div>
  );
}
...
function TeamBlock({ team, dim, reverse }) {
  return (
    <div className={`flex flex-1 flex-col items-center gap-1 ${dim ? 'opacity-40' : ''}`}>
