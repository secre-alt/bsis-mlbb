...
function BracketBoard({ state, getTeam, seedMap, isAdmin, onSubmit }) {
  ...
  return <>
    <section className="relative mt-2 h-[280px] overflow-hidden border border-[var(--line)] bg-[var(--panel)] sm:h-[330px] md:h-[390px]" style={{ minHeight: 280 }}>
      ...
      <svg className="pointer-events-none absolute inset-0 z-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><path d="M 38 31 H 50 V 50 H 60" fill="none" stroke="rgba(255,90,31,.85)" strokeWidth=".5" /><path d="M 38 69 H 50 V 50" fill="none" stroke="rgba(255,90,31,.85)" strokeWidth=".5" /><circle cx="50" cy="50" r=".9" fill="#ff5a1f" /></svg>
...
function FixtureRow({ team, label, score }) { return <div className="flex h-8 items-center gap-2 border border-[var(--line)] bg-[var(--panel-strong)] px-2 shadow-[0_4px_12px_var(--shadow)]">{team && <TeamLogo team={team} size={18} fontSize={5} />}<span className="min-w-0 flex-1 truncate text-[10px] font-black">{label || team?.abbr || "TBD"}</span><span className="w-5 text-right font-display text-sm font-black text-ember-500">{score ?? "-"}</span></div>; }
...
function ResultEditor({ match, getTeam, onSubmit }) {
  const [games, setGames] = useState([null, null, null]); const [message, setMessage] = useState(""); const [revision, setRevision] = useState(match.updatedAt);
  ...
  const saveGame = async (index, winner) => { const nextGames = games.map((game, i) => i === index ? winner : game); setGames(nextGames); setMessage("Saving live score..."); const { error, updatedAt } = await onSubmit({ slot: match.slot, gameResults: nextGames, updatedAt: revision }); if (error) setMessage(error); else { setRevision(updatedAt || revision); setMessage("Live score saved"); } };
  return <section className="mt-4 border border-[var(--line)] bg-[var(--panel)] p-4"><div className="mb-3 flex items-center justify-between"><h2 className="text-[10px] font-black uppercase tracking-[.16em] text-ember-500">Record {match.slot === "grand_final" ? "Grand Final" : "Semifinal"}</h2><span className="text-[9px] font-bold uppercase text-[var(--text-muted)]">Save each game live</span></div><div className="grid gap-1.5 sm:max-w-lg">{[0, 1, 2].map((index) => <div key={index} className="grid grid-cols-[34px_1fr_1fr] gap-1"><span className="py-1.5 text-[10px] text-[var(--text-muted)]">G{index + 1}</span><button disabled={decidedBefore(index)} onClick={() => saveGame(index, "A")} className={`border py-1.5 text-[10px] font-bold disabled:opacity-30 ${games[index] === "A" ? "border-ember-500 bg-ember-500/10 text-ember-500" : "border-[var(--line)]"}`}>{teamA?.abbr}</button><button disabled={decidedBefore(index)} onClick={() => saveGame(index, "B")} className={`border py-1.5 text-[10px] font-bold disabled:opacity-30 ${games[index] === "B" ? "border-ember-500 bg-ember-500/10 text-ember-500" : "border-[var(--line)]"}`}>{teamB?.abbr}</button></div>)}</div>{message && <p className="mt-3 text-[10px] text-[var(--text-muted)]">{message}</p>}</section>;
}
