...
function LegacyResultEditor({ match, getTeam, onSubmit }) {
  const [games, setGames] = useState([null, null, null, null, null]); const [message, setMessage] = useState(""); const [revision, setRevision] = useState(match.updatedAt); const [saving, setSaving] = useState(false);
  useEffect(() => { setGames(match.gameResults?.length ? [...match.gameResults, null, null, null, null].slice(0, 5) : [null, null, null, null, null]); setMessage(""); setRevision(match.updatedAt); }, [match]);
  const teamA = getTeam(match.teamA); const teamB = getTeam(match.teamB);
  const decidedBefore = (index) => games.slice(0, index).filter((game) => game === "A").length === 3 || games.slice(0, index).filter((game) => game === "B").length === 3;
  ...
  const recordedGames = games.filter(Boolean);
  const undoLastGame = async () => {
    if (saving || recordedGames.length === 0 || !window.confirm(`Undo Game ${recordedGames.length}?`)) return;
    const previousGames = games;
    const nextGames = [...recordedGames.slice(0, -1), null, null, null, null].slice(0, 5);
    setGames(nextGames); setSaving(true); setMessage("Undoing last game...");
    try { const { error, updatedAt } = await onSubmit({ slot: match.slot, gameResults: nextGames, updatedAt: revision }); if (error) { setGames(previousGames); setMessage(error); } else { setRevision(updatedAt || revision); setMessage("Last game undone"); } } catch { setGames(previousGames); setMessage("Unable to undo the last game. Try again."); } finally { setSaving(false); }
  };
  return <section className="mt-4 border border-[var(--line)] bg-[var(--panel)] p-4"><div className="mb-3 flex items-center justify-between"><h2 className="text-[10px] font-black uppercase tracking-[.16em] text-ember-500">Record {match.slot === "grand_final" ? "Grand Final" : "Semifinal"}</h2><span className="text-[9px] font-bold uppercase text-[var(--text-muted)]">BO5 · Save each game live</span></div><div className="border-t border-[var(--line)] pt-3"><div className="grid gap-1.5 sm:max-w-lg">{[0, 1, 2, 3, 4].map((index) => <div key={index} className="grid grid-cols-[34px_1fr_1fr] gap-1"><span className="py-1.5 text-[10px] text-[var(--text-muted)]">G{index + 1}</span><button disabled={gameLocked(index)} onClick={() => saveGame(index, "A")} className={`border py-1.5 text-[10px] font-bold disabled:opacity-30 ${games[index] === "A" ? "border-ember-500 bg-ember-500/10 text-ember-500" : "border-[var(--line)]"}`}>{teamA?.abbr}</button><button disabled={gameLocked(index)} onClick={() => saveGame(index, "B")} className={`border py-1.5 text-[10px] font-bold disabled:opacity-30 ${games[index] === "B" ? "border-ember-500 bg-ember-500/10 text-ember-500" : "border-[var(--line)]"}`}>{teamB?.abbr}</button></div>)}</div></div>{message && <p className="mt-3 text-[10px] text-[var(--text-muted)]">{message}</p>}</section>;
}
function ResultEditor({ match, getTeam, onSubmit }) {
  const [games, setGames] = useState([null, null, null, null, null]);
  const [message, setMessage] = useState("");
  const [revision, setRevision] = useState(match.updatedAt);
  ...
  const decidedBefore = (index) => games.slice(0, index).filter((game) => game === "A").length === 3 || games.slice(0, index).filter((game) => game === "B").length === 3;
  const gameLocked = (index) => saving || Boolean(games[index]) || (index > 0 && games.slice(0, index).some((game) => !game)) || decidedBefore(index);
  const submitGames = async (nextGames, successMessage, pendingMessage) => {
    const previousGames = games; setGames(nextGames); setSaving(true); setMessage(pendingMessage);
    try { const { error, updatedAt } = await onSubmit({ slot: match.slot, gameResults: nextGames, updatedAt: revision }); if (error) { setGames(previousGames); setMessage(error); } else { setRevision(updatedAt || revision); setMessage(successMessage); } } catch { setGames(previousGames); setMessage("Unable to save the live score. Try again."); } finally { setSaving(false); }
  };
  const saveGame = (index, winner) => { if (!gameLocked(index)) submitGames(games.map((game, i) => i === index ? winner : game), "Live score saved", "Saving live score..."); };
  const undoLastGame = () => {
    if (saving || recordedGames.length === 0 || !window.confirm(`Undo Game ${recordedGames.length}?`)) return;
    submitGames([...recordedGames.slice(0, -1), null, null, null, null].slice(0, 5), "Last game undone", "Undoing last game...");
  };
  return <section className="mt-4 border border-[var(--line)] bg-[var(--panel)] p-4"><div className="mb-3 flex items-center justify-between"><h2 className="text-[10px] font-black uppercase tracking-[.16em] text-ember-500">Record {match.slot === "grand_final" ? "Grand Final" : "Semifinal"}</h2><span className="text-[9px] font-bold uppercase text-[var(--text-muted)]">BO5 · Save each game live</span></div><div className="border-t border-[var(--line)] pt-3"><div className="grid gap-1.5 sm:max-w-lg">{[0, 1, 2, 3, 4].map((index) => <div key={index} className="grid grid-cols-[34px_1fr_1fr] gap-1"><span className="py-1.5 text-[10px] text-[var(--text-muted)]">G{index + 1}</span><button disabled={gameLocked(index)} onClick={() => saveGame(index, "A")} className={`border py-1.5 text-[10px] font-bold disabled:opacity-30 ${games[index] === "A" ? "border-ember-500 bg-ember-500/10 text-ember-500" : "border-[var(--line)]"}`}>{teamA?.abbr}</button><button disabled={gameLocked(index)} onClick={() => saveGame(index, "B")} className={`border py-1.5 text-[10px] font-bold disabled:opacity-30 ${games[index] === "B" ? "border-ember-500 bg-ember-500/10 text-ember-500" : "border-[var(--line)]"}`}>{teamB?.abbr}</button></div>)}</div><button type="button" disabled={saving || recordedGames.length === 0} onClick={undoLastGame} className="mt-3 border border-blood-500/60 px-3 py-1.5 text-[10px] font-bold text-blood-500 disabled:opacity-30">Undo last game</button></div>{message && <p className="mt-3 text-[10px] text-[var(--text-muted)]">{message}</p>}</section>;
}
