...
export default function AdminPanel({ teams, matches, submitMatchResult, scheduleMatch, onDone }) {
  ...
  const handleSubmitResult = async (e) => {
    ...
    const winner = teams.find(
      (t) => t.id === Number(result.scoreA > result.scoreB ? result.teamA : result.teamB)
    );
    showToast(`Match ${result.num} recorded · ${winner?.abbr} wins ${result.scoreA}–${result.scoreB}`, 'success');
    onDone?.();
  };
  ...
  return (
    <div className="mx-auto max-w-xl px-5 py-6">
      ...
      <form onSubmit={handleSubmitResult} className="mb-8 rounded-md border border-ink-800 bg-ink-900 p-6">
        <Row>
          <Field label="Match #">
            <input type="number" min={1} className="input" value={result.num}
              onChange={(e) => setResult((r) => ({ ...r, num: e.target.value }))} />
          </Field>
          <Field label="Round">
            <input type="number" min={1} className="input" value={result.round}
              onChange={(e) => setResult((r) => ({ ...r, round: e.target.value }))} />
          </Field>
        </Row>
        <Row>
          <Field label="Date">
            <input type="date" className="input" value={result.date}
              onChange={(e) => setResult((r) => ({ ...r, date: e.target.value }))} />
          </Field>
          <Field label="Time">
            <input className="input" value={result.time}
              onChange={(e) => setResult((r) => ({ ...r, time: e.target.value }))} />
          </Field>
        </Row>
        <div className="mb-3">
          <Field label="Team A">
            <select className="input" value={result.teamA}
              onChange={(e) => setResult((r) => ({ ...r, teamA: e.target.value }))}>
              {teamOptions}
            </select>
          </Field>
        </div>
        <div className="mb-4">
          <Field label="Team B">
            <select className="input" value={result.teamB}
              onChange={(e) => setResult((r) => ({ ...r, teamB: e.target.value }))}>
              {teamOptions}
            </select>
          </Field>
        </div>
        ...
        <div className="mb-4 rounded-sm border border-ink-800 bg-ink-950 p-4">
          ...
          <Row>
            <ScoreField
              label="Team A score"
              value={result.scoreA}
              onChange={(v) => setResult((r) => ({ ...r, scoreA: clampScore(v) }))}
            />
            <ScoreField
              label="Team B score"
              value={result.scoreB}
              onChange={(v) => setResult((r) => ({ ...r, scoreB: clampScore(v) }))}
            />
          </Row>
        </div>

        {resultError && <div className="mb-3 text-xs font-medium text-blood-500">{resultError}</div>}
      ...
      <form onSubmit={handleSchedule} className="rounded-md border border-ink-800 bg-ink-900 p-6">
        <Row>
          <Field label="Match #">
            <input type="number" min={1} className="input" value={sched.num}
              onChange={(e) => setSched((s) => ({ ...s, num: e.target.value }))} />
          </Field>
          <Field label="Round">
            <input type="number" min={1} className="input" value={sched.round}
              onChange={(e) => setSched((s) => ({ ...s, round: e.target.value }))} />
          </Field>
        </Row>
        <Row>
          <Field label="Date">
          ...
          <Field label="Time">
            <input className="input" value={sched.time}
              onChange={(e) => setSched((s) => ({ ...s, time: e.target.value }))} />
          </Field>
        </Row>
        <div className="mb-3">
          <Field label="Team A">
            <select className="input" value={sched.teamA}
              onChange={(e) => setSched((s) => ({ ...s, teamA: e.target.value }))}>
              {teamOptions}
            </select>
          </Field>
        </div>
        <div className="mb-4">
          <Field label="Team B">
            <select className="input" value={sched.teamB}
              onChange={(e) => setSched((s) => ({ ...s, teamB: e.target.value }))}>
              {teamOptions}
            </select>
          </Field>
        </div>
  ...
  return (
    <Field label={label}>
      <div className="flex gap-2">
        <button type="button" onClick={() => onChange(Number(value) - 1)} className="btn-step">
          <Minus size={15} />
        </button>
        <input
          type="number"
