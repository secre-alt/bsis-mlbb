...
import { TeamLogo, SectionLabel, EmptyState, SkeletonBlock, FormDots } from './shared';
...
export default function Standings({ teams, matches, getTeam, loading }) {
  ...
  const latest = matches.filter((m) => m.status === 'completed').slice(-3).reverse();
    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-5 py-6 md:grid-cols-[1fr_320px]">
          ...
          <div className="overflow-hidden rounded-md border border-ink-800">
            ...
            {standings.map((s, i) => {
              ...
              return (
                <div
                  key={s.id}
                  className={`relative grid grid-cols-[auto_1fr_auto] items-center gap-3 border-t border-ink-800 px-4 py-3 first:border-t-0 hover:bg-ink-800/50 sm:grid-cols-[32px_1fr_32px_32px_32px_36px_36px_40px_48px_44px] sm:gap-1 sm:py-0 sm:min-h-[52px] ${
                    rank === 1 ? 'bg-ember-500/[0.06]' : ''
                  }`}
                >
                  <span
                    className={`absolute left-0 top-0 h-full w-[3px] ${
                      rank === 1 ? 'bg-ember-500' : rank === 2 ? 'bg-ink-500' : rank === 3 ? 'bg-amber-800' : 'bg-transparent'
                    }`}
                  />
                  <div className={`text-center text-sm font-bold ${rank === 1 ? 'text-ember-500' : 'text-ink-500'}`}>
                    {rank}
                  </div>
                  <div className="flex min-w-0 items-center gap-2.5">
                    <TeamLogo team={team} />
                    <div className="min-w-0">
                      <div className="truncate text-xs font-bold">{team.abbr}</div>
                      <div className="truncate text-[10px] text-ink-500">{team.name}</div>
                    </div>
                  </div>
                  <Cell hideOnMobile>{s.mp}</Cell>
                  <Cell hideOnMobile className="text-rift-500">{s.w}</Cell>
                  <Cell hideOnMobile className="text-blood-500">{s.l}</Cell>
                  <Cell hideOnMobile>{s.gw}</Cell>
                  <Cell hideOnMobile>{s.gl}</Cell>
                  <Cell hideOnMobile className={s.diff > 0 ? 'text-rift-500' : s.diff < 0 ? 'text-blood-500' : 'text-ink-600'}>
                    {s.diff > 0 ? `+${s.diff}` : s.diff}
                  </Cell>
                  <div className="hidden justify-center sm:flex">
                    <FormDots form={s.form} />
                  </div>
                  <div className="text-center text-sm font-extrabold text-ember-500">{s.pts}</div>
                </div>
              );
            })}
          </div>
      ...
      <div>
        ...
        <div className="mb-6 space-y-2.5">
          {latest.length ? (
            latest.map((m) => <ResultCard key={m.id} match={m} getTeam={getTeam} />)
          ) : (
        ...
        <div className="space-y-2.5">
          {upcoming.length ? (
            upcoming.map((m) => <UpcomingCard key={m.id} match={m} getTeam={getTeam} />)
          ) : (
...
function Cell({ children, className = '', hideOnMobile }) {
  return (
    <div className={`text-center text-xs font-semibold text-ink-300 ${hideOnMobile ? 'hidden sm:block' : ''} ${className}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="relative overflow-hidden rounded-md border border-ink-800 bg-ink-900 p-3.5">
      ...
      <div className="mb-2 flex justify-between font-mono text-[9px] font-bold uppercase tracking-wider text-ink-600">
        <span>Match {m.num} · R{m.round}</span>
      </div>
...
function TeamCol({ team, score, winner }) {
  return (
    <div className="flex flex-1 flex-col items-center gap-1">
      ...
      <div className={`text-[11px] font-bold ${winner ? 'text-white' : 'text-ink-600'}`}>{team.abbr}</div>
      <div className={`font-display text-xl font-bold ${winner ? 'text-ember-500' : 'text-ink-700'}`}>{score}</div>
    </div>
  );
}
...
function UpcomingCard({ match: m, getTeam }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-md border border-ink-800 bg-ink-900 p-3.5">
      ...
      <div className="mt-2 text-center text-[9px] font-bold uppercase tracking-wider text-ember-500">
        Upcoming · {m.date || ''}
      </div>
    </div>
  );
